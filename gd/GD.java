        ...
        if(answer.equals("simple") || answer.equals("Simple") || answer.equals("SIMPLE")){
            ...
            do{
                for(int j = 0; j < ds_simple.getY().length; j++){
                    y_hat[j] = Betha[1]*ds_simple.getX()[j]+Betha[0];//Aquí se aplican los valores de entrada a la predicción
                    y_pred[j] = ds_simple.getY()[j]-y_hat[j];//diferencia_y = (y-ý)(ye menos ye de hat)   
